...
import com.cg.LoanServiceApplication;
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            ...
            if (!jwtUtil.isTokenExpired(token)) {
                ...
                if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                    ...
                    throw new LoanServiceException("Invalid JWT token", 401);
                }
            } else {
                throw new LoanServiceException("JWT token is expired", 401);
            }
        } else {
            // Commented out exception handling for missing token
             throw new LoanServiceException("Authorization JWT Token is missing in Header", 401);
        }
        chain.doFilter(request, response);
    }
